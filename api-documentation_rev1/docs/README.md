...
Before proceeding with Klarna integration, make sure you have Klarna enabled as a payment method in the Paytrail [Merchant Panel](https://merchant.paytrail.com).

Klarna can be integrated in two ways. You may choose one or use a combination of both:

- **Hosted payment page or webshop rendered payment buttons** (standard integration)
...
Although not mandatory, we recommended passing as much customer and item information as possible when using Klarna as a payment method
...
Providing complete and correct data improves acceptance rates for Klarna.

### Conversion booster's

You can optionally use Klarna [Conversion Booster's](https://docs.klarna.com/conversion-boosters/) with your payments. To do so include [klarna provider details](#Klarna-provider-details) in the payment request [providerDetails](#providerDetails) object.
Both of these fields are required when using this feature.

Using klarna conversion boosters requires you to obtain and handle session token on your own. Paytrail only provides a way to pass it along with your payments.
...
| ------------ | ------ | ------------------ | ------- | ------------------------------ |
| sessionToken | string | <center>x</center> |         | Min Lenght 1, Max lenght 8192  |
| data         | string | <center>x</center> |         | Min Lenght 1, Max lenght 10240 |
...
With `manualInvoiceActivation` you are issueing an authorization hold for the amount. **It is advised** to issue a cancel for the payment as soon as you know you are going to do so, even though this is an automatic operation at the end of expiry window.
